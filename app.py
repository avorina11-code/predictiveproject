...
def render_courbe_projection(df: pd.DataFrame, obj_sl: float):
    ...
    labels = df["tranche"].tolist()
    sl_cumule = df["sl_cumule"].round(1).tolist()
    sl_projete = df["sl_max_projete"].round(1).tolist()
    objectif = [round(obj_sl, 1)] * len(df)
...
def styliser_dataframe(df_aff: pd.DataFrame, obj_sl: float, obj_qs: float, dmt_cible: float,
    ...
    styler = (
        df_aff.style
        .applymap(lambda v: couleur_pct(v, obj_sl), subset=["SL %", "SL Cumulé %", "SL Max Projeté %"])
        .applymap(lambda v: couleur_pct(v, obj_qs), subset=["QS %", "TRP %"])
        .applymap(couleur_dmt, subset=["DMT (S)"])
        .applymap(couleur_effectif, subset=["Écart Effectif"])
        .applymap(couleur_statut, subset=["Statut"])
...
k4.metric("Heure de bascule", synthese["heure_bascule"] if synthese["heure_bascule"] else "Non atteinte")
